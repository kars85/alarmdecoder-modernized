...
def get_event_description(event_type: int, event_code: int) -> str:
    ...
    # We need to use Dict[Any, Any] or simply 'dict' to avoid type errors
    lookup_map = LRR_TYPE_MAP.get(event_type, None)

    if lookup_map is not None:
        result = lookup_map.get(event_code, description)
        # Extract the real message if it's a dictionary
        if isinstance(result, dict):
            description = result.get('message', description)
...
def get_event_data_type(event_type: int, event_code: int) -> str:
    ...
    if lookup_map is not None:
        result = lookup_map.get(event_code, dtype)
...
LRR_ADEMCO_MAP: dict[int, str | dict[str, str]] = {
...
LRR_UNKNOWN_MAP: dict[int, str | dict[str, str]] = {

}

# Map of event type codes to text maps.
LRR_TYPE_MAP: dict[int, dict[int, Any]] = {
